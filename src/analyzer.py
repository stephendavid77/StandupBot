...
class SprintAnalyzer:
    ...
    def analyze(self, project_config, sprint):
        ...
        if not issues:
            print(f"No issues found in sprint {sprint.name} for project {project_key}. Skipping analysis.")
            return None
        ...
        if cancelled_statuses:
            ...
            print(f"Filtered out {initial_issue_count - len(issues)} cancelled issues. Remaining issues: {len(issues)}.")
        ...
        issues_completed_after_sprint_end = self._analyze_issues_completed_after_sprint_end(issues, datetime.strptime(sprint.endDate.split("T")[0], "%Y-%m-%d"))
        
        print("Analyzing issues removed from sprint...")
        issues_removed_from_sprint = self._analyze_issues_removed_from_sprint(sprint.id, project_key, issues)
    ...
    def _analyze_issues_completed_after_sprint_end(self, issues, sprint_end_date):
        ...
        for issue in issues:
            if issue.fields.resolutiondate:
                resolution_date = datetime.strptime(issue.fields.resolutiondate.split("T")[0], "%Y-%m-%d").date()
                if resolution_date > sprint_end_date.date():
                    completed_after_sprint.append(issue.key)
        return completed_after_sprint

    def _analyze_issues_removed_from_sprint(self, sprint_id, project_key, issues_in_sprint):
        removed_issues = []
        ...
        for issue in all_sprint_issues:
            ...
            if issue.key not in current_sprint_issue_keys and \
        ...
        if not epic_link_field_id:
            ...
            for field_id, field_name in field_mappings.items():
                if field_name == 'Epic Link':
                    ...
                    epic_link_field_id = field_id
                    print("Warning: Using 'Epic Name' field as 'Epic Link' was not found. This might not be the correct field for linking epics.")
                    break

        if not epic_link_field_id:
            print("Warning: 'epic_link_field' not configured in config.yaml and could not be determined from Jira fields. Cannot extract epic information.")
            return {}
        ...
        if not story_points_field_id:
            for field_id, field_name in field_mappings.items():
                if field_name == 'Story point estimate' or field_name == 'Story Points':
                    ...
                    break
            if not story_points_field_id:
                print("Warning: 'story_points_field' not configured in config.yaml and could not be determined from Jira fields. Story points will not be calculated.")
                story_points_field_id = 'customfield_10016' # Fallback to a common default
    ...
    def _check_fix_version_hygiene(self, issues, guidelines):
        ...
        return {
            "issues_without_fix_versions": {
                ...
                "is_issue": fix_version_hygiene_pct > guidelines.get('max_issues_without_fix_versions_pct', 0)
            }
        }
        ...
        if not story_points_field_id:
            for field_id, field_name in field_mappings.items():
                if field_name == 'Story point estimate' or field_name == 'Story Points':
                    ...
                    break
            if not story_points_field_id:
                print("Warning: 'story_points_field' not configured in config.yaml and could not be determined from Jira fields. Story points will not be calculated for workload analysis.")
                story_points_field_id = 'customfield_10016' # Fallback to a common default
