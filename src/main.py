...
from email_report import email_report # Add this import
...
def _load_jira_fields(config, jira_client):
    ...
    if reload_fields or not jira_fields:
        ...
        try:
            ...
            print(f"Error reloading Jira fields from API: {e}. Using cached fields if available.")
...
@click.option('--report-type', '-r', type=click.Choice(['daily', 'full', 'previous']), default=None, help='Override the report type from config.')
...
def run(report_type, export, skip_pdf):
    ...
    for project_config in config.get('projects', []):
        ...
        print(f"Preparing analysis for project: {project_key} (Board ID: {board_id})...")
        ...
        if not sprint_to_analyze:
            print(f"No sprint found for analysis for project {project_key} with report type '{final_report_type}'. Skipping.")
            continue # Skip to the next project

        print(f"Analyzing sprint: {sprint_to_analyze.name} for project: {project_key}...")
        ...
        if results:
            ...
            if final_export:
                ...
                reporter_instance.export_report(report_content, filename=filename, formats=formats)
                        ...
                        if recipient_emails:
                            ...
                            for recipient in recipient_emails:
                                ...
                                if email_success:
                                    ...
                                    print(f"Failed to send email report to {recipient}.")
                        else:
                            print("email_recipient_email not set in config.yaml. Skipping email report.")
                    else:
                        print(f"Report file for email not found at {report_to_email}. Skipping email.")
            else:
                print("Email report not sent as export is disabled.")
        else:
            print(f"No analysis results for project: {project_key}. Skipping report generation.")
