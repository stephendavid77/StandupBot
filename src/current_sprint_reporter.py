...
class CurrentSprintReporter(Reporter): # Inherit from Reporter
    ...
    def generate_daily_summary(self):
        ...
        generated_time = now_est.strftime("%H:%M:%S %Z%z") # %Z for timezone name, %z for UTC offset
    ...
    def generate_full_report(self):
        ...
        if not hygiene['unassigned_issues']['is_issue'] and \
           not hygiene['unestimated_issues']['is_issue'] and \
           not hygiene['issues_without_fix_versions']['is_issue']:
    ...
    def _generate_issues_without_fix_versions_table(self):
        ...
        all_issues = self.results.get('all_issues', []) # Assuming 'all_issues' contains the full issue objects
        ...
        for fmt in formats:
            if fmt == 'md':
                ...
                try:
                    ...
                    subprocess.run([str(Path(sys.executable).parent / 'mdpdf'), str(md_file_path), '-o', str(pdf_file_path)], check=True)
