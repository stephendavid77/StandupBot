...
class JiraClient:
    ...
    def _connect(self):
        ...
        if not jira_email:
            raise ConnectionError("Jira email not found in environment variables or config.yaml. Please set JIRA_EMAIL environment variable or 'jira_email' in config.yaml.")
        
        if not api_token:
            print("API token not found in environment variables or config.yaml. Attempting to retrieve from keyring...")
            try:
                import keyring
                ...
                for backend_path in non_interactive_backends:
                    ...
                    try:
                        ...
                        current_keyring_path = current_keyring.__class__.__module__ + '.' + current_keyring.__class__.__name__
                        ...
                        try:
                            ...
                            print(f"Backend {backend_path} is not available or not configured.")
                            continue
                        ...
                        print(f"Could not set or use keyring backend {backend_path}: {e}")
                        continue
                
                if not backend_set:
                    raise ConnectionError("Could not find a suitable non-interactive keyring backend. Please ensure 'keyrings.alt' and 'cryptography' are installed (`pip install keyrings.alt cryptography`) or provide 'api_token' in config.yaml.")

                api_token = keyring.get_password("StandupBot", jira_email)
                if not api_token:
                    raise ConnectionError(f"API token not found in keyring for user {jira_email}. Please set it using 'keyring.set_password(\"StandupBot\", \"{jira_email}\", \"YOUR_JIRA_PAT\")')")
                print("API token retrieved from keyring.")
            except ImportError as ie:
                raise ConnectionError("Keyring library not found. Please install it (`pip install keyring`) or provide 'api_token' in config.yaml.")
            except Exception as e:
                raise ConnectionError(f"Error retrieving API token from keyring: {e}")
        
        print(f"API Token found. Attempting Jira authentication for user: {jira_email}...")

        try:
            jira_options = {'server': server}
            jira = JIRA(options=jira_options, basic_auth=(jira_email, api_token))
            if not jira.myself():
                 raise ConnectionError("Could not connect to Jira. Please check your email, API token, and server URL.")
    ...
    def get_last_closed_sprint(self, board_id):
        ...
        try:
            ...
            print(f"Last closed sprint found: {last_closed_sprint.name} (Ended: {last_closed_sprint.endDate})")
    ...
    def get_issues_not_in_sprint(self, project_key, sprint_id):
        print(f"Fetching issues not in sprint for project {project_key} and sprint {sprint_id}...\n")
